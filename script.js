...
let totalRecebimentos = 2195.00; // Valor igual ao mostrado no HTML
let aLiberar = 380.00; // Valor igual ao mostrado no HTML
let bankAccounts = [
    { banco: 'Banco do Brasil', agencia: '1234-5', conta: '67890-1', tipoConta: 'Corrente' },
    { banco: 'Caixa Econômica Federal', agencia: '2468-0', conta: '13579-8', tipoConta: 'Poupança' }
...
function showScreen(screenId, title = '') {
    ...
    // Adiciona a renderização para as abas de orçamentos do usuário
        ...
        console.log('Abrindo tela financeiro - currentUserType:', currentUserType);
        console.log('totalRecebimentos:', totalRecebimentos, 'aLiberar:', aLiberar);
        updateFinanceiroDashboard();
        renderBankAccounts();
        console.log('Tela financeiro inicializada');
...
function updateFinanceiroDashboard() {
    console.log('updateFinanceiroDashboard executada - valores:', totalRecebimentos, aLiberar);
    const elementoTotalRecebimentos = document.getElementById('total-recebimentos');
    const elementoALiberar = document.getElementById('a-liberar');
    console.log('Elementos encontrados:', elementoTotalRecebimentos, elementoALiberar);
    
    if (elementoTotalRecebimentos) {
        elementoTotalRecebimentos.textContent = `R$ ${totalRecebimentos.toFixed(2).replace('.', ',')}`;
        console.log('Total recebimentos atualizado para:', elementoTotalRecebimentos.textContent);
    }
    if (elementoALiberar) {
        elementoALiberar.textContent = `R$ ${aLiberar.toFixed(2).replace('.', ',')}`;
        console.log('A liberar atualizado para:', elementoALiberar.textContent);
    }
}
...
function renderBankAccounts() {
    console.log('renderBankAccounts executada - bankAccounts:', bankAccounts);
    const container = document.getElementById('bank-accounts-container');
    console.log('Container bank-accounts-container encontrado:', container);
    if (!container) return;
    container.innerHTML = '';
    ...
    bankAccounts.forEach((account, index) => {
        ...
        item.innerHTML = `
            ...
            <div class="account-actions">
                <button class="btn btn-small" onclick="editBankAccount(${index})"><i class="fas fa-edit"></i> Editar</button>
                <button class="btn btn-finalizar btn-small" onclick="deleteBankAccount(${index})"><i class="fas fa-trash"></i> Remover</button>
            </div>
        `;
        container.appendChild(item);
        console.log('Conta bancária renderizada:', account.banco);
    });
}
...
// Função para mostrar/ocultar horário personalizado em datas específicas
function toggleHorarioPersonalizado() {
  const personalizado = document.getElementById('tipo-personalizado');
  const horarioSection = document.getElementById('horario-personalizado');
  
  if (personalizado && personalizado.checked) {
    horarioSection.style.display = 'block';
  } else {
    horarioSection.style.display = 'none';
  }
...
function adicionarDataEspecifica() {
  const dataInput = document.getElementById('data-especifica');
  const tipoInput = document.querySelector('input[name="tipo-data"]:checked');
  const listaElement = document.getElementById('lista-datas-especificas');
  ...
  if (!tipoInput) {
    alert('Por favor, selecione o tipo da data.');
    return;
  }
  ...
  if (tipo === 'bloqueado') {
    descricao = 'Bloqueado - não receber orçamentos';
  } else if (tipo === 'personalizado') {
    const inicioInput = document.getElementById('data-inicio');
    const fimInput = document.getElementById('data-fim');
    descricao = `Horário personalizado: ${inicioInput.value} às ${fimInput.value}`;
  }
  
  // Verifica se a lista está vazia (primeira data)
  if (listaElement.children.length === 1 && listaElement.children[0].textContent.includes('Nenhuma data')) {
    listaElement.innerHTML = '';
  }
  
  // Adiciona a nova data à lista
  const dataDiv = document.createElement('div');
  dataDiv.className = 'data-especifica-item';
  dataDiv.innerHTML = `
    <p><strong>${new Date(data).toLocaleDateString('pt-BR')}:</strong> ${descricao}</p>
    <button class="btn btn-small" onclick="this.parentElement.remove()">Remover</button>
  `;
  
  listaElement.appendChild(dataDiv);
  
  // Limpa o formulário
  dataInput.value = '';
  document.getElementById('tipo-bloqueado').checked = true;
  document.getElementById('horario-personalizado').style.display = 'none';
  
  alert('Data específica adicionada com sucesso!');
}

// Função para salvar calendário de trabalho
function salvarCalendarioTrabalho() {
  alert('Calendário de trabalho salvo com sucesso!');
  console.log('Calendário salvo');
}

// Função para gerar resumo do calendário
function gerarResumoCalendario() {
  const resumoElement = document.getElementById('resumo-calendario');
  const resumoSection = document.getElementById('resumo-section');
  
  if (resumoElement) {
    resumoElement.innerHTML = `
      <div class="resumo-item">
        <h4>Modo de Orçamento</h4>
        <p>Configuração atual do recebimento de orçamentos</p>
      </div>
      <div class="resumo-item">
      ...
      <div class="resumo-item">
        <h4>Feriados</h4>
        <p>Configuração para dias de feriado</p>
      </div>
    `;
  }
...
function resetarCalendario() {
  if (confirm('Tem certeza que deseja resetar todas as configurações para o padrão?')) {
    // Resetar para modo "sempre"
    document.getElementById('modo-sempre').checked = true;
    document.getElementById('modo-horario-comercial').checked = false;
    document.getElementById('modo-calendario').checked = false;
    
    // Chamar a função para ocultar seções
    handleModoOrcamentoChange('sempre');
    
    alert('Calendário resetado para configurações padrão!');
  }
}
